...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      { title: "Sign up — HourClips" },
      { name: "description", content: "Create your HourClips account and start turning podcasts into viral clips." },
    ],
  }),
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        ...
        <div className="border border-border bg-surface/70 backdrop-blur p-8 rounded-2xl">
          ...
          <p className={`text-sm text-muted-foreground mb-8 ${ar ? "font-arabic" : ""}`}>{t("auth.signup.sub")}</p>

          <button
            onClick={signInGoogle}
            disabled={busy}
            className="w-full bg-primary text-primary-foreground py-4 font-bold uppercase tracking-tighter hover:brightness-110 transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-3 rounded-lg"
          >
